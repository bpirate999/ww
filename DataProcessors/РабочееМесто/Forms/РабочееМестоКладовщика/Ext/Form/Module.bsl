	...
	Если ДокПроизводствоБезЗаказа.Проведен Тогда
		...
		Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура") И Не ДанныеЗаполнения.Свойство("ЗаполнитьПоОтбору") Тогда	
			...
			Попытка
				...
				Если ДокВыработкаСотрудников.Проведен Тогда
					
					ДанныеЗаполнения = Новый Структура;
					ДанныеЗаполнения.Вставить("ЗаполнениеКладовщиком");
					ДанныеЗаполнения.Вставить("Организация", Обработка.Организация);
					ДанныеЗаполнения.Вставить("ВыработкаСотрудников", ДокВыработкаСотрудников.Ссылка);
					ДанныеЗаполнения.Вставить("РаботникиТЧ", Обработка.Работники);
					ДанныеЗаполнения.Вставить("ВидДокумента", 
					
					ИнтерфейсРаботникаСклада.ПолучитьСсылкуПоГуид("Справочник.ВидыДокументовВводДанныхДляРасчетаЗарплаты", 
					"cfe80946-4290-11eb-a216-ac1f6be2fed5"));
					
					ДанныеЗаполнения.Вставить("Период", Обработка.Дата); 
					ДанныеЗаполнения.Вставить("ПериодОкончания", Обработка.Дата);
					
					ДокДанныеДляРасчетаЗарплаты = Документы.ДанныеДляРасчетаЗарплаты.СоздатьДокумент();
					ДокДанныеДляРасчетаЗарплаты.Заполнить(ДанныеЗаполнения); 
					
					Если ДокДанныеДляРасчетаЗарплаты.ВыполненныеРаботы.Количество() Тогда
						
						Попытка
							
							ДокДанныеДляРасчетаЗарплаты.Записать(РежимЗаписиДокумента.Проведение);
							
						Исключение 
							
							ДокДанныеДляРасчетаЗарплаты.Записать(РежимЗаписиДокумента.Запись);
							
						КонецПопытки;
						
						СформированныеДокументы.Добавить(ДокДанныеДляРасчетаЗарплаты.Ссылка);	
...
Процедура ЗаливкаУпаковкаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка) 
	...
	Оповещение = Новый ОписаниеОповещения("ВыборИзСпискаУпаковокЗавершение", ЭтаФорма);
	
	СписокУпаковок.ПоказатьВыборЭлемента(Оповещение, "Газоблоки");	
	
КонецПроцедуры

&НаКлиенте
Процедура ВыборИзСпискаУпаковокЗавершение(Элемент, ДопПар) Экспорт
